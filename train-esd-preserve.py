...
def train_esd(prompt, train_method, start_guidance, negative_guidance, iterations, lr, config_path, ckpt_path, diffusers_config_path, devices, seperator=None, image_size=512, ddim_steps=50, info=''):
    ...
    name = f'compvis-word_{word_print}-method_{train_method}-sg_{start_guidance}-ng_{negative_guidance}-iter_{iterations}-lr_{lr}-info_{info}'
    # TRAINING CODE
    pbar = tqdm(range(iterations))
    for i in pbar:
        word = random.sample(erased_words,1)[0]
        preserved_word = random.sample(preserved_words,1)[0]
        ...
        emb_preserved = model.get_learned_conditioning([preserved_word])
        ...
        loss += criteria(e_preserved.to(devices[0]), e_preserved_org.to(devices[0]))
