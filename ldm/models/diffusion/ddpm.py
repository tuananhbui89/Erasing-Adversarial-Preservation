...
#                 self.cond_stage_model.train = disabled_train
    ...
    def apply_model_with_prompt(self, x_noisy, t, cond, prompt=None, return_ids=False):

        """
        this function will call DiffusionWrapper. self.model is DiffusionWrapper not the UNetModel
        """
        ...
        x_recon = self.model.forward_with_prompt(x_noisy, t, prompt=prompt, **cond)
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward_with_prompt(self, x, t, prompt, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        ...
        if prompt['mode'] == 'mid':
            func = self.diffusion_model.forward_with_prompt
        elif prompt["mode"] == 'inmidout':
            func = self.diffusion_model.forward_with_prompt_inmidout
        elif prompt["mode"] == 'midout':
            func = self.diffusion_model.forward_with_prompt_midout

        _prompt = prompt['data']

        if self.conditioning_key is None:
            out = func(x, t, prompt=_prompt)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1)
            out = func(xc, t, prompt=_prompt)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            out = func(x, t, context=cc, prompt=_prompt)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = func(xc, t, context=cc, prompt=_prompt)
        elif self.conditioning_key == 'hybrid-adm':
            assert c_adm is not None
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = func(xc, t, context=cc, y=c_adm, prompt=_prompt)
        elif self.conditioning_key == 'adm':
            cc = c_crossattn[0]
            out = func(x, t, y=cc, prompt=_prompt)
        else:
            raise NotImplementedError()
