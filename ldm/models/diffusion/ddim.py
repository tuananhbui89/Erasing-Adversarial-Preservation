...
class DDIMSampler(object):
    ...
    def ddim_sampling(self, cond, shape, prompt=None,
        ...
        for i, step in enumerate(iterator):
            ...
            outs = self.p_sample_ddim(img, cond, ts, prompt=prompt, index=index, use_original_steps=ddim_use_original_steps,
    ...
    def p_sample_ddim(self, x, c, t, index, prompt=None, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
        ...
        if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
            if prompt is None:
                ...
                e_t = self.model.apply_model_with_prompt(x, t, c, prompt=prompt)
        else:
            x_in = torch.cat([x] * 2)
            t_in = torch.cat([t] * 2)
...
#             print(f'Input shape before model: {x_in.shape} {t_in.shape}')
            if prompt is None:
                ...
                e_t_uncond, e_t = self.model.apply_model_with_prompt(x_in, t_in, c_in, prompt=prompt).chunk(2)
