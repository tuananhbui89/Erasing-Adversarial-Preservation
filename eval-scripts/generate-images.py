...
def generate_images(model_name, prompts_path, save_path, device='cuda:0', guidance_scale = 7.5, image_size=512, ddim_steps=100, num_samples=10, from_case=0, to_case=None):
    '''
    ...
    if model_name not in ['SD-v1-4','SD-V2','SD-V2-1']:
        try:
            if '.pt' in model_name:
                ...
                unet.load_state_dict(torch.load(f'{models_path}/{model_name}.pt'))
            else:
...
if __name__=='__main__':
    ...
    generate_images(model_name, prompts_path, save_path, device=device,
                    guidance_scale = guidance_scale, image_size=image_size, ddim_steps=ddim_steps, num_samples=num_samples,from_case=from_case, to_case=to_case)
